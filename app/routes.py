...
def form_hh():
    ...
    if form_hh.validate_on_submit():
        city = str(form_hh.city.data).strip().capitalize()
        print('city: ', city)
        ...
        if id:
            ...
            # print(stat.get_stat())
            # flash('Login requested for user {}, city {}, remember_me={}'.format(
            # form_hh.username.data, form_hh.city.data, form_hh.remember_me.data))
            return redirect(url_for('results'))
        else:
            flash('Такой город не найден {}'.format(form_hh.city.data))
...
def results():
    # posts = stat.get_stat()
    posts = head_hanter.stat_orm2.get_stat_SQLite()
    print(posts)
    return render_template('results.html', title='Результаты', posts=posts)
